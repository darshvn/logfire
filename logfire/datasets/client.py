...
def _serialize_case(case: 'Case[Any, Any, Any]') -> dict[str, Any]:
    ...
    data: dict[str, Any] = {
        'inputs': _serialize_value(case.inputs) if not isinstance(case.inputs, dict) else case.inputs,
    }
    ...
    if case.metadata is not None:
        data['metadata'] = (
            _serialize_value(case.metadata)
...
class LogfireDatasetsClient(_BaseLogfireDatasetsClient[Client]):
    ...
    def create_case(
        ...
        *,
        ...
        Raises:
            DatasetNotFoundError: If the dataset does not exist.
        """
        serialized_inputs = _serialize_value(inputs) if not isinstance(inputs, dict) else inputs
    ...
    def export_dataset(
        ...
        *,
        ...
        return typed_dataset_cls.from_dict(data, custom_evaluator_types=list(custom_evaluator_types))
    ...
    def import_cases(
        ...
        if cases and hasattr(cases[0], 'inputs'):
            ...
            serialized_cases = [_serialize_case(case) for case in cases]  # type: ignore
        else:
            # Already dicts
            serialized_cases = list(cases)  # type: ignore
